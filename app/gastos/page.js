...
export default function GastosOperativosPage() {
  ...
  if (rolUsuario === 'facturacion') {
    return (
      <div className="flex bg-slate-950 min-h-screen text-slate-200 w-full"><Sidebar /><main className="flex-1 p-8 flex flex-col items-center justify-center"><h2 className="text-2xl text-white font-black uppercase tracking-widest">Acceso Restringido</h2></main></div>
    );
  }
      ...
      <main className="flex-1 p-8 overflow-y-auto custom-scrollbar">
        <div className="max-w-[1400px] mx-auto">
          ...
          <header className="flex justify-between items-center mb-10">
            <div>
              <h1 className="text-3xl font-black text-white italic uppercase tracking-tighter">Egresos <span className="text-blue-600">Operativos</span></h1>
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <button onClick={() => setMostrarFiltro(!mostrarFiltro)} className={`flex items-center gap-3 border px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filtroActivo ? 'bg-blue-600/10 border-blue-500/30 text-blue-400' : 'bg-slate-900 border-slate-800 text-slate-400 hover:text-white'}`}>
                  <Calendar size={14} className={filtroActivo ? 'text-blue-500' : 'text-slate-500'} /> {filtroActivo ? 'Filtros Activos' : 'Periodo y Reportes'} <ChevronDown size={14} />
                </button>
                  ...
                  <div className="absolute right-0 mt-3 w-80 bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-2xl z-50 animate-in fade-in zoom-in-95">
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        <div><label className="text-[9px] font-black text-slate-500 uppercase block mb-2">Desde</label><input type="date" className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-xs text-white outline-none focus:border-blue-500" value={fechaInicio} onChange={(e) => setFechaInicio(e.target.value)} /></div>
                        <div><label className="text-[9px] font-black text-slate-500 uppercase block mb-2">Hasta</label><input type="date" className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-xs text-white outline-none focus:border-blue-500" value={fechaFin} onChange={(e) => setFechaFin(e.target.value)} /></div>
                      </div>
                      <div className="pt-4 border-t border-slate-800 space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                           {filtroActivo && (<button onClick={() => { setFiltroActivo(false); setFechaInicio(primerDiaMes); setFechaFin(ultimoDiaMes); setMostrarFiltro(false); }} className="bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-colors">Limpiar</button>)}
                           <button onClick={exportarGastosExcel} disabled={loading || historial.length === 0} className={`${filtroActivo ? '' : 'col-span-2'} flex items-center justify-center gap-2 bg-emerald-600/10 hover:bg-emerald-600 text-emerald-500 hover:text-white border border-emerald-500/20 font-black text-[9px] uppercase tracking-widest py-2.5 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed`}><FileText size={14} /> Excel</button>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <button onClick={abrirModalCrear} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all shadow-lg flex items-center gap-2"><PlusCircle size={14} /> Registrar Gasto</button>
            </div>
          </header>
            ...
            <div className="overflow-x-auto custom-scrollbar pb-2">
              <table className="w-full text-left border-collapse text-[13px] min-w-[1100px]">
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {historial.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-slate-800/40 transition-colors group">
                        <td className="p-4 pl-8 whitespace-nowrap"><span className="text-[14px] text-white font-mono font-medium">{item.folio_interno ? `G-${String(item.folio_interno).padStart(4, '0')}` : 'G-S/N'}</span></td>
                        <td className="p-4 whitespace-nowrap">
                          {vieneDeViaje ? ( <span className="inline-flex px-2 py-1 rounded-md bg-blue-900/30 border border-blue-500/30 text-blue-400 font-mono text-[12px] items-center gap-1.5"><Truck size={12}/> V-{String(item.viajes?.folio_interno).padStart(4, '0')}</span>) : ( <span className="text-slate-600 text-[12px] font-mono">---</span> )}
                        </td>
                        <td className="p-4 whitespace-nowrap">
                          {item.unidades?.numero_economico ? ( <span className="inline-flex px-2 py-1 rounded-md bg-slate-800 border border-slate-700 text-slate-300 font-mono text-[12px]">ECO: {item.unidades.numero_economico}</span> ) : ( <span className="text-slate-600 text-[12px] font-mono">---</span> )}
                        </td>
                        <td className="p-4 whitespace-nowrap"><span className="text-[12px] text-slate-400 font-medium">{item.fecha?.slice(0, 10) || 'S/F'}</span></td>
                        <td className="p-4 whitespace-normal min-w-[350px]">
                          <div className="flex flex-col items-start py-1">
                            <span className="text-slate-200 text-sm leading-tight">{item.descripcion}</span>
                            <span className="text-slate-500 text-[11px] mt-1 font-medium">{(item.gastos_detalle?.length || 0)} {(item.gastos_detalle?.length === 1) ? 'concepto registrado' : 'conceptos registrados'}</span>
                          </div>
                        </td>
                        <td className="p-4 whitespace-nowrap text-right"><span className="text-[15px] font-mono font-black text-white">${Number(item.costo).toLocaleString('es-MX', {minimumFractionDigits: 2})}</span></td>
                        <td className="p-4 pr-8 whitespace-nowrap text-center">
                          <div className="flex items-center justify-center gap-2 opacity-30 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => abrirModalEditar(item)} title="Editar" className="p-2 transition-colors rounded-lg text-slate-400 hover:text-blue-400 hover:bg-blue-500/10"><Edit size={16} /></button>
                            <button onClick={() => eliminarGasto(item.id)} title="Eliminar" className="p-2 transition-colors rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-500/10"><Trash2 size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    );
                  ...
                  {historial.length === 0 && (
                    <tr><td colSpan="7" className="py-16 text-center"><Wallet size={32} className="mx-auto text-slate-700 mb-3" /><p className="text-slate-500 uppercase tracking-widest text-sm font-black">Sin registros en este periodo</p></td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          {mostrarFormulario && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <div className="relative bg-slate-900 border border-slate-800 w-full max-w-3xl rounded-[3rem] p-10 shadow-2xl animate-in zoom-in-95 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <h2 className="text-2xl font-black text-white italic uppercase mb-8 shrink-0">{editandoId ? 'Editar' : 'Captura de'} <span className="text-blue-500">Gasto</span></h2>
                ...
                <form onSubmit={registrarGasto} className="space-y-6 overflow-y-auto pr-2 custom-scrollbar">
                  <div className="bg-slate-950/50 p-6 rounded-3xl border border-slate-800 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <select className="w-full bg-slate-900 border border-slate-800 p-3.5 rounded-xl text-sm text-white focus:border-blue-500 outline-none" value={formData.unidad_id} onChange={e => setFormData({...formData, unidad_id: e.target.value})}>
                        <option value="">-- Sin Unidad Específica --</option>
                        {unidades.map(u => <option key={u.id} value={u.id}>ECO: {u.numero_economico}</option>)}
                      </select>
                      <select className="w-full bg-slate-900 border border-slate-800 p-3.5 rounded-xl text-sm text-white focus:border-blue-500 outline-none" value={formData.viaje_id} onChange={e => setFormData({...formData, viaje_id: e.target.value})}>
                      ...
                      <input type="date" className="w-full bg-slate-900 border border-slate-800 p-3.5 rounded-xl text-sm text-white focus:border-blue-500 outline-none" value={formData.fecha} onChange={e => setFormData({...formData, fecha: e.target.value})} required />
                    </div>
                    <input required className="w-full bg-slate-900 border border-slate-800 p-3.5 rounded-xl text-sm text-white focus:border-blue-500 outline-none" value={formData.descripcion_general} onChange={e => setFormData({...formData, descripcion_general: e.target.value})} placeholder="Concepto General (Ej. Gastos de Viaje MTY-QRO)" />
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-end border-b border-slate-800 pb-2">
                      ...
                      <button type="button" onClick={agregarLinea} className="text-[10px] font-black text-blue-400 hover:text-blue-300 uppercase tracking-widest flex items-center gap-1 bg-blue-500/10 px-3 py-1.5 rounded-lg border border-blue-500/20 transition-colors"><PlusCircle size={12} /> Agregar Concepto</button>
                    </div>
                    ...
                    <div className="max-h-[250px] overflow-y-auto custom-scrollbar space-y-3 pr-2">
                      {formData.lineas.map((linea) => (
                        <div key={linea.id_temp} className="flex flex-col sm:flex-row gap-3 bg-slate-950 p-3 rounded-2xl border border-slate-800 group hover:border-slate-700 transition-colors">
                          <select required className="sm:w-1/4 bg-slate-900 border border-slate-800 p-3 rounded-xl text-xs text-white focus:border-blue-500 outline-none" value={linea.tipo} onChange={e => actualizarLinea(linea.id_temp, 'tipo', e.target.value)}>
                          ...
                          <input required className="flex-1 bg-slate-900 border border-slate-800 p-3 rounded-xl text-xs text-white focus:border-blue-500 outline-none" value={linea.descripcion} onChange={e => actualizarLinea(linea.id_temp, 'descripcion', e.target.value)} placeholder="Descripción del gasto..." />
                          <div className="sm:w-1/4 relative">
                            ...
                            <input required type="number" step="0.01" className="w-full bg-slate-900 border border-slate-800 p-3 pl-7 pr-10 rounded-xl text-xs text-white font-mono focus:border-blue-500 outline-none" value={linea.monto} onChange={e => actualizarLinea(linea.id_temp, 'monto', e.target.value)} placeholder="0.00" />
                            {formData.lineas.length > 1 && (
                              <button type="button" onClick={() => eliminarLinea(linea.id_temp)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
                            )}
                          </div>
                        </div>
                      ))}
                  ...
                  <div className="pt-4 border-t border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div className="text-left w-full sm:w-auto">
                      ...
                      <p className="text-3xl font-mono font-black text-blue-500 italic">${totalLiquidacion.toLocaleString('es-MX', {minimumFractionDigits: 2})}</p>
                    </div>
                    <button type="submit" disabled={loading || formData.lineas.length === 0} className="w-full sm:w-auto flex-1 bg-blue-600 text-white py-4 px-8 rounded-2xl font-black uppercase text-[11px] tracking-widest shadow-xl hover:bg-blue-500 transition-all flex justify-center items-center gap-3 disabled:opacity-50">

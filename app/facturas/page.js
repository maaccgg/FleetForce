...
function FacturasContenido() {
  ...
  const exportarExcelFacturas = () => {
    const datosParaExcel = historial.map(f => ({ Folio_Interno: f.folio_interno ? `FF-${f.folio_interno}` : 'FF-S/N', Folio_Viaje: f.viaje_id || f.folio_viaje ? `V-${f.folio_viaje}` : 'N/A', Fecha_Emision: f.fecha_viaje, Fecha_Vencimiento: f.fecha_vencimiento || 'S/V', Estatus_Pago: f.estatus_pago, Fecha_Pago_Real: f.fecha_pago || 'Pendiente', Cliente: f.cliente, Concepto: f.ruta || '', Referencia: f.referencia || '', Monto_Total: f.monto_total, Moneda: f.moneda || 'MXN', Metodo_Pago: f.metodo_pago, Forma_Pago: f.forma_pago, UUID_SAT: f.folio_fiscal || 'Sin Timbrar' }));
                    ...
                    return (
                      <tr key={item.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group ${esCancelada ? 'opacity-40 grayscale' : ''}`}>
                        ...
                        <td className="p-4 align-middle">
                          ...
                          <span className="text-[14px] text-slate-900 dark:text-white font-mono font-medium transition-colors">{item.folio_interno ? `FF-${item.folio_interno}` : 'FF-S/N'}</span>            
{vieneDeViaje ? ( <span className="inline-flex px-2 py-0.5 rounded bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-500/30 text-blue-600 dark:text-blue-400 uppercase tracking-widest text-[9px] items-center gap-1 mt-0.5 transition-colors"><Truck size={8}/> VIAJE: {item.folio_viaje ? `V-${item.folio_viaje}` : 'V-S/N'}</span> ) : ( <span className="inline-flex px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[9px] items-center gap-1 mt-0.5 transition-colors">Libre</span> )}
                          </div>
                        </td>
